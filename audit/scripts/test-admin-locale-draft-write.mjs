...
function fixture({ locale = null, currentItem = item(), claims = { version: 1, claims: [] }, reservations = { version: 1, reservations: [] }, missingItem = false, branchConflict = false } = {}) {
  ...
  const client = {
    ...
    async createBlob(content) { calls.blobs.push(content); return { sha: calls.blobs.length.toString(16).padStart(40, 'a') }; },
...
{ const test = fixture(); const input = createInput({ payload: payload('crow', 'Crow', { alphabet_key: 'Z', based_on_source_revision: 999 }) }); const original = structuredClone(input.payload); const result = await createLocaleDraftWrite(test.client, input); assert.equal(result.changed, true); assert.equal(result.localeBlobSha, result.transaction.blobShas.get(PATHS.locale)); assert.notEqual(result.localeBlobSha, result.transaction.blobShas.get(DRAFT_CLAIMS_PATH)); assert.match(result.localeBlobSha, /^[0-9a-f]{40}$/); assert.equal(result.localeDocument.draft.based_on_source_revision, 7); assert.equal(result.localeDocument.draft.alphabet_key, 'Z'); assert.deepEqual(input.payload, original); assert.deepEqual(changedPaths(test.calls), [PATHS.locale, DRAFT_CLAIMS_PATH]); assert.equal(test.calls.trees[0].entries.find((entry) => entry.path === PATHS.locale).sha !== null, true); assert.equal(test.calls.refs, 1); assert.equal(test.calls.commits, 1); assert.equal(test.calls.reads.length, 5); assert.deepEqual(test.calls.updates, [{ branch: 'main', sha: 'commit-next', force: false }]); }
{ const originalGet = Map.prototype.get; Map.prototype.get = function (key) { return key === PATHS.locale && this.size <= 2 && this.has(PATHS.locale) ? undefined : originalGet.call(this, key); }; try { await code('LOCALE_BLOB_SHA_MISSING', () => createLocaleDraftWrite(fixture().client, createInput())); } finally { Map.prototype.get = originalGet; } }
...
{ const test = fixture({ locale: document({ draft: localized() }), claims: { version: 1, claims: [{ locale: 'en', slug: 'crow', content_id: A }] } }); const result = await updateLocaleDraftWrite(test.client, { branch: 'main', contentId: A, locale: 'en', payload: payload(), expectedLocaleBlobSha: 'locale-sha' }); assert.equal(result.changed, false); assert.equal(result.code, 'NO_CHANGES'); assert.equal(result.localeBlobSha, 'locale-sha'); assert.equal(test.calls.trees.length, 0); }

{ const old = { ...localized('crow', 'Crow', { based_on_source_revision: 7, based_on_source_fingerprint: FP7 }), version: 1, published_at: '2026-09-15' }; const test = fixture({ currentItem: item(8, FP8), locale: document({ published: old }) }); const result = await beginLocaleEditWrite(test.client, { branch: 'main', contentId: A, locale: 'en', expectedLocaleBlobSha: 'locale-sha' }); assert.equal(result.localeBlobSha, result.transaction.blobShas.get(PATHS.locale)); assert.deepEqual(changedPaths(test.calls), [PATHS.locale]); assert.equal(result.localeDocument.draft.based_on_source_revision, 7); assert.deepEqual(result.localeDocument.published, old); }
...
{ const draft = localized('crow', 'Crow', { based_on_source_revision: 7, based_on_source_fingerprint: FP7 }); const test = fixture({ currentItem: item(8, FP8), locale: document({ draft }), claims: { version: 1, claims: [{ locale: 'en', slug: 'crow', content_id: A }] } }); const result = await rebaseLocaleDraftWrite(test.client, { branch: 'main', contentId: A, locale: 'en', expectedLocaleBlobSha: 'locale-sha' }); assert.equal(result.localeBlobSha, result.transaction.blobShas.get(PATHS.locale)); assert.deepEqual(changedPaths(test.calls), [PATHS.locale]); assert.equal(result.localeDocument.draft.based_on_source_revision, 8); assert.equal(result.localeDocument.draft.content, draft.content); }
{ const test = fixture({ locale: document({ draft: localized() }), claims: { version: 1, claims: [{ locale: 'en', slug: 'crow', content_id: A }] } }); const result = await rebaseLocaleDraftWrite(test.client, { branch: 'main', contentId: A, locale: 'en', expectedLocaleBlobSha: 'locale-sha' }); assert.equal(result.code, 'NO_CHANGES'); assert.equal(result.localeBlobSha, 'locale-sha'); assert.equal(test.calls.trees.length, 0); }
...
{ const test = fixture({ locale: document({ draft: localized() }), claims: { version: 1, claims: [{ locale: 'en', slug: 'crow', content_id: A }] } }); const result = await discardLocaleDraftWrite(test.client, { branch: 'main', contentId: A, locale: 'en', expectedLocaleBlobSha: 'locale-sha' }); assert.equal(result.localeBlobSha, null); assert.deepEqual(changedPaths(test.calls), [PATHS.locale, DRAFT_CLAIMS_PATH]); assert.equal(test.calls.trees[0].entries.find((entry) => entry.path === PATHS.locale).sha, null); }
{ const published = { ...localized(), version: 1, published_at: '2026-09-15' }; const test = fixture({ locale: document({ draft: localized(), published }) }); const result = await discardLocaleDraftWrite(test.client, { branch: 'main', contentId: A, locale: 'en', expectedLocaleBlobSha: 'locale-sha' }); assert.equal(result.localeBlobSha, result.transaction.blobShas.get(PATHS.locale)); assert.deepEqual(changedPaths(test.calls), [PATHS.locale]); }
