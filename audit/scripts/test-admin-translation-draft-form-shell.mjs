...
assert.match(source, /data-rebase-action[^>]*hidden[^>]*disabled[^>]*>Mark reviewed for current HY<\/button>/);
assert.match(source, /data-discard-action[^>]*hidden[^>]*disabled[^>]*>Discard Draft<\/button>/);
...
assert.match(source, /rebaseAction\.hidden = !\(\(state === 'DRAFT' \|\| state === 'PUBLISHED_WITH_DRAFT'\).*draft_synchronization === 'OUTDATED'/s);
assert.match(source, /rebaseAction\.disabled = saving \|\| writeBlocked \|\| isDirty\(\) \|\| !canRebase\(\)/);
assert.equal(/<(?:button|a)[^>]*>\s*Rebase\s*</i.test(source), false);
