...
assert.match(source, /rebaseHelp\.hidden = rebaseAction\.hidden/);
assert.match(source, /rebaseDirtyHelp\.hidden = rebaseAction\.hidden \|\| !isDirty\(\)/);
assert.match(source, /draft_synchronization === 'OUTDATED'/);
