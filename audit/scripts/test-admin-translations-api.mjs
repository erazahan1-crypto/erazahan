...
function installFetch({ missing = false, malformed = false, transportFailure = false, allowWrites = false, locale = null, currentItem = item, claims = { version: 1, claims: [] }, reservations = { version: 1, reservations: [] }, branchConflict = false } = {}) {
  ...
  const blobs = new Map(); const createdBlobContents = []; let index = 0;
  ...
  globalThis.fetch = async (input, init = {}) => {
    ...
    if (init.method && init.method !== 'GET') {
      ...
      if (path.endsWith('/git/blobs')) { createdBlobContents.push(JSON.parse(init.body).content); return json({ sha: `${(calls.filter((call) => call.path.endsWith('/git/blobs')).length).toString(16).padStart(40, 'a')}` }); }
  ...
  return { calls, trees, createdBlobContents, restore: () => { globalThis.fetch = original; } };
}
...
const postEnv = { ...env, ERAZAHAN_LOCALE_ADMIN_ATOMIC_BRANCH: 'main', ERAZAHAN_ADMIN_DEPLOYMENT_CLASS: 'production' };
...
for (const guardedEnv of [
  { ...postEnv, ERAZAHAN_ADMIN_DEPLOYMENT_CLASS: 'preview' },
  { ...env, ERAZAHAN_LOCALE_ADMIN_ATOMIC_BRANCH: 'main' },
  { ...postEnv, ERAZAHAN_ADMIN_DEPLOYMENT_CLASS: 'unknown' },
]) { const mock = installFetch(); try { const res = await post(createBody(), { env: guardedEnv }); assert.equal(res.status, 503); assert.equal(mock.calls.length, 0); } finally { mock.restore(); } }
...
const UNKNOWN_CONTENT = '11111111-1111-5111-8111-111111111111';
function logicalDraft(content, image_alts = {}) { return { ...localized(), content, image_alts }; }
for (const [body, fixture, expected] of [
  [createBody({ payload: { ...payload('crow', 'Crow', {}), content: `[missing](content://${UNKNOWN_CONTENT})` } }), {}, 'Referenced content_id does not exist'],
  [createBody({ payload: { ...payload('crow', 'Crow', {}), content: `![](asset://${UNKNOWN_ASSET})` } }), {}, 'Referenced asset_id does not exist'],
  [publishBody(), { locale: localeDocument({ draft: logicalDraft(`![](asset://${BEXER_ASSET})`) }), claims: { version: 1, claims: [{ locale: 'en', slug: 'crow', content_id: A }] } }, 'Published asset requires image_alts'],
  [publishBody(), { locale: localeDocument({ draft: logicalDraft(`[self](content://${A})`) }), claims: { version: 1, claims: [{ locale: 'en', slug: 'crow', content_id: A }] } }, 'cannot link to itself'],
]) { const mock = installFetch(fixture); try { const res = await post(body, { env: postEnv }); const result = await res.json(); assert.equal(res.status, 400); assert.ok(['DRAFT_INVALID', 'PUBLISH_INVALID'].includes(result.code)); assert.match(result.message, new RegExp(expected)); } finally { mock.restore(); } }
...
{ const currentItem = { ...item, source_revision: 2, source_fingerprint: 'c'.repeat(64) }; const draft = { ...localized('crow-at-dawn', 'Crow at Dawn'), description: 'A detailed draft description', content: '<p>Detailed <strong>draft</strong> content.</p>', image_alts: { hero: 'Crow on a branch', detail: 'Morning mist' }, tags: ['Birds', 'Morning'], alphabet_key: 'C', updated_at: '2026-09-17T12:34:56Z', generation: { kind: 'human', generated_at: '2026-09-17T12:34:56Z' } }; const draftBefore = structuredClone(draft); const published = { ...localized('crow-at-dawn', 'Published Crow'), description: 'Published description', content: '<p>Published content.</p>', image_alts: { hero: 'Published crow' }, tags: ['Published'], alphabet_key: 'P', version: 6, published_at: '2026-09-12' }; const input = localeDocument({ draft, published }); const mock = installFetch({ allowWrites: true, currentItem, locale: input, reservations: { version: 1, reservations: [{ locale: 'en', slug: 'crow-at-dawn', content_id: A, kind: 'active' }] } }); try { const res = await post(rebaseBody(), { env: postEnv }); const body = await res.json(); assert.equal(res.status, 200); assert.equal(body.ok, true); assert.equal(body.changed, true); for (const field of ['slug', 'title', 'description', 'content', 'image_alts', 'tags', 'alphabet_key']) assert.deepEqual(body.locale_document.draft[field], draftBefore[field], `API rebase preserves draft ${field}`); assert.deepEqual(body.locale_document.draft, expectedRebasedDraft(draftBefore, 2, 'c'.repeat(64)), 'API rebase changes only draft provenance'); assert.equal(body.locale_document.draft.updated_at, draftBefore.updated_at); assert.deepEqual(body.locale_document.draft.generation, draftBefore.generation); assert.deepEqual(body.locale_document.published, published, 'API rebase preserves published snapshot'); assert.equal(body.locale_document.published.version, 6, 'API rebase preserves published version'); assert.equal(deriveTranslationState(input, item).publication_state, 'PUBLISHED_WITH_DRAFT'); assert.equal(deriveTranslationState(body.locale_document, currentItem).publication_state, 'PUBLISHED_WITH_DRAFT', 'API rebase preserves lifecycle'); assert.deepEqual(JSON.parse(mock.createdBlobContents[0]), body.locale_document, 'API response matches the locale document committed by the transaction'); assert.match(body.locale_blob_sha, /^[0-9a-f]{40}$/); } finally { mock.restore(); } }
{ const currentItem = { ...item, source_revision: 2, source_fingerprint: 'c'.repeat(64) }; const draft = { ...localized('draft-crow', 'Draft Crow'), description: 'A draft-only description', content: '<p>Draft-only <em>content</em>.</p>', image_alts: { hero: 'Draft crow' }, tags: ['Draft', 'Birds'], alphabet_key: 'D', updated_at: '2026-09-17T13:14:15Z', generation: { kind: 'human', generated_at: '2026-09-17T13:14:15Z' } }; const draftBefore = structuredClone(draft); const input = localeDocument({ draft }); const validClaim = { version: 1, claims: [{ locale: 'en', slug: 'draft-crow', content_id: A }] }; const request = rebaseBody(); assert.deepEqual(request, { action: 'rebase', content_id: A, locale: 'en', expected_locale_blob_sha: localeSha }, 'draft-only rebase request shape is exact'); const mock = installFetch({ allowWrites: true, currentItem, locale: input, claims: validClaim }); try { const res = await post(request, { env: postEnv }); const body = await res.json(); assert.equal(res.status, 200); assert.equal(body.ok, true); assert.equal(body.changed, true); assert.deepEqual(body.locale_document.draft, expectedRebasedDraft(draftBefore, 2, 'c'.repeat(64)), 'draft-only API rebase changes only provenance'); for (const field of ['slug', 'title', 'description', 'content', 'image_alts', 'tags', 'alphabet_key']) assert.deepEqual(body.locale_document.draft[field], draftBefore[field], `draft-only API rebase preserves ${field}`); assert.equal(body.locale_document.draft.updated_at, draftBefore.updated_at); assert.deepEqual(body.locale_document.draft.generation, draftBefore.generation); assert.equal(body.locale_document.published, null); assert.equal(deriveTranslationState(input, item).publication_state, 'DRAFT'); assert.equal(deriveTranslationState(body.locale_document, currentItem).publication_state, 'DRAFT', 'draft-only API rebase preserves lifecycle'); assert.deepEqual(JSON.parse(mock.createdBlobContents[0]), body.locale_document, 'draft-only API response matches committed locale document'); } finally { mock.restore(); } }
{ const mock = installFetch({ locale: existingDraft, claims: { version: 1, claims: [{ locale: 'en', slug: 'crow', content_id: A }] } }); try { const res = await post(rebaseBody(), { env: postEnv }); const body = await res.json(); assert.equal(res.status, 200); assert.equal(body.ok, true); assert.equal(body.changed, false); assert.equal(body.code, 'NO_CHANGES'); assert.deepEqual(body.locale_document, existingDraft); assert.equal(body.locale_blob_sha, localeSha); assert.equal(mock.calls.some((call) => call.method !== 'GET'), false); assert.equal(mock.createdBlobContents.length, 0); assert.equal(mock.trees.length, 0); } finally { mock.restore(); } }
