...
try {
  ...
  const ruXml = serializeSitemapXml(listLocaleSitemapEntries(scanContentStore(fixture({ ru: published('ru', 'ворона-во-сне') })), 'ru'), origin);
  assert.deepEqual(xmlUrls(ruXml), ['https://erazahan.info/ru/ворона-во-сне/']);
  const enXml = serializeSitemapXml(listLocaleSitemapEntries(scanContentStore(fixture({ en: published('en', 'crow-dream-meaning') })), 'en'), origin);
  assert.deepEqual(xmlUrls(enXml), ['https://erazahan.info/en/crow-dream-meaning/']);
  assert.deepEqual(listLocaleSitemapEntries(scanContentStore(fixture({ ru: draft('ru', 'черновик'), en: draft('en', 'draft') })), 'ru'), []);
  assert.deepEqual(listLocaleSitemapEntries(scanContentStore(fixture({ ru: draft('ru', 'черновик'), en: draft('en', 'draft') })), 'en'), []);

  const publishedDraft = serializeSitemapXml(listLocaleSitemapEntries(scanContentStore(fixture({ ru: both('ru', 'секретная-ворона', 'старая-ворона') })), 'ru'), origin);
  ...
  const outdatedRu = listLocaleSitemapEntries(scanContentStore(fixture({ ru: published('ru', 'старая-ворона', { based_on_source_revision: 2 }) })), 'ru');
  const outdatedEn = listLocaleSitemapEntries(scanContentStore(fixture({ en: published('en', 'old-crow', { based_on_source_revision: 2 }) })), 'en');
