...
function payload(locale, slug, fingerprint, overrides = {}) {
  return { slug: locale === 'ru' ? ruFixtureSlug(slug) : slug, title: `${locale} title`, description: null, content: `<p>${locale} content</p>`, image_alts: {}, tags: [locale], alphabet_key: null, based_on_source_revision: locale === 'hy' ? null : 1, based_on_source_fingerprint: locale === 'hy' ? null : fingerprint, ...overrides };
}
...
try {
  ...
  { const fixture = root(); add(fixture, A, { ru: published('ru', 'ru-live') }); const routes = listLocalizedDreamRouteEntries(scanContentStore(fixture), 'ru'); assert.equal(routes.length, 1); assert.equal(routes[0].slug, ruFixtureSlug('ru-live')); assert.equal(routes[0].path, `/ru/${ruFixtureSlug('ru-live')}/`); assert.equal(JSON.stringify(routes[0]).includes('draft'), false); }
  { const fixture = root(); add(fixture, A, { ru: both('ru', 'ru-future', 'ru-old') }); const routes = listLocalizedDreamRouteEntries(scanContentStore(fixture), 'ru'); assert.equal(routes[0].path, `/ru/${ruFixtureSlug('ru-old')}/`); assert.equal(routes.some((route) => route.slug === ruFixtureSlug('ru-future')), false); }
  { const fixture = root(); add(fixture, A, { ru: both('ru', 'secret-new-slug', 'public-old-slug') }); const file = `${A.slice(0, 2)}/${A}/ru.json`; const doc = JSON.parse(readFileSync(path.join(fixture, file), 'utf8')); doc.published.title = 'PUBLIC TITLE'; doc.published.content = 'PUBLIC CONTENT'; doc.draft.title = 'SECRET DRAFT TITLE'; doc.draft.content = 'SECRET DRAFT CONTENT'; write(fixture, file, doc); const route = listLocalizedDreamRouteEntries(scanContentStore(fixture), 'ru')[0]; const serialized = JSON.stringify({ entry: route.entry }); for (const secret of [ruFixtureSlug('secret-new-slug'), 'SECRET DRAFT TITLE', 'SECRET DRAFT CONTENT', '"draft"']) assert.equal(serialized.includes(secret), false); for (const publicValue of [ruFixtureSlug('public-old-slug'), 'PUBLIC TITLE', 'PUBLIC CONTENT']) assert.equal(serialized.includes(publicValue), true); assert.equal(route.slug, ruFixtureSlug('public-old-slug')); }
