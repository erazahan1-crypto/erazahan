...
function validateRecord(record) {
  ...
  for (const key of Object.keys(record)) if (!RECORD_KEYS.has(key)) fail(`asset record.${key} is not allowed`);
  try { assertAssetId(record.asset_id); } catch (error) { fail(error instanceof Error ? error.message : 'asset_id is invalid'); }
  ...
  for (const dimension of ['width', 'height']) if (Object.hasOwn(record, dimension) && (!Number.isInteger(record[dimension]) || record[dimension] < 1)) fail(`${dimension} must be a positive integer`);
...
export function createMediaManifestIndex(manifest) {
  ...
  if (manifest.schema_version !== MEDIA_MANIFEST_SCHEMA_VERSION || !Array.isArray(manifest.assets) || Object.keys(manifest).length !== 2) fail('manifest must contain schema_version 1 and assets');
...
export function resolveLogicalAssetReference(mediaIndex, assetId) {
  try { assertAssetId(assetId); } catch (error) { fail(error instanceof Error ? error.message : 'asset_id is invalid'); }
